        ...
        <div className="flex h-full min-h-0 flex-col rounded-[20px] border border-slate-200/80 bg-white/95 p-3 text-slate-700 shadow-[0_18px_42px_rgba(15,23,42,0.075)] backdrop-blur-xl">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1 className="mt-0.5 truncate text-sm font-black tracking-tight text-slate-950">Panel académico</h1>
            </div>
          ...
          <nav className="sidebar-nav mt-5 min-h-0 flex-1 space-y-4 overflow-y-auto overflow-x-hidden pr-1" aria-label="Secciones del estudiante">
            {navGroups.map((group) => (
              <div key={group.label}>
          ...
          <footer className="mt-auto border-t border-slate-200/75 pt-3">
            <div className="flex items-center gap-2.5 rounded-2xl bg-slate-50/75 p-2">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-black text-slate-900">{session?.nombreCompleto ?? 'Estudiante'}</p>
                <p className="mt-0.5 text-xs font-semibold text-slate-500">{session?.rol ?? 'Estudiante'}</p>
              </div>
            </div>
