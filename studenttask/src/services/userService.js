...
const updateStoredUserPassword = (profileOrEmail, password) => {
  const source = typeof profileOrEmail === 'string' ? { email: profileOrEmail } : profileOrEmail ?? {};
  const profileEmail = normalizeEmailValue(source.email);
  const profileId = Number(source.id);
