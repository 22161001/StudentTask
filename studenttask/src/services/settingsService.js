...
import { getRoleHomePath, normalizeRoleKey } from '../utils/roles';
...
const getDefaultPrivateRoute = (role) => {
  const hasExplicitRole = role !== undefined && role !== null && String(role).trim();

  if (hasExplicitRole) {
    const roleKey = normalizeRoleKey(role);
    return roleKey === 'alumno' ? '/dashboard' : getRoleHomePath(role);
  }

  return defaultRouteMap[getSettings().vistaDefault] ?? '/dashboard';
};
