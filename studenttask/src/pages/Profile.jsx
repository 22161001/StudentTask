...
import { FiBookOpen, FiLock, FiMail, FiSave, FiUser } from 'react-icons/fi';
import FeedbackBanner from '../components/FeedbackBanner';
import MainLayout from '../layout/MainLayout';
import PageHero from '../components/PageHero';
import SectionCard from '../components/SectionCard';
import { getProfile, syncProfile, updatePassword, updateProfile } from '../services/profileService';
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      if (result.ok) {
        ...
        setFeedback({ type: 'error', message: result.message || 'No se pudo cargar el perfil.' });
      }
  ...
  const handlePasswordChange = (event) => {
    ...
    setPasswordErrors((currentErrors) => ({ ...currentErrors, [name]: '' }));
  ...
  const handleSubmit = async (event) => {
    ...
    setSaved(true);
    setFeedback({ type: result.fallback ? 'info' : 'success', message: result.message || 'Perfil actualizado.' });
    setSubmitting(false);
  };
  ...
  const handlePasswordSubmit = async (event) => {
    ...
    if (!result.ok) {
      ...
      setPasswordFeedback({ type: 'error', message: result.message ?? 'Revisa los campos marcados.' });
    ...
    setPasswordFeedback({ type: result.fallback ? 'info' : 'success', message: result.message || 'Contraseña actualizada.' });
    setChangingPassword(false);
  };
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <FeedbackBanner type="info" message="Cargando perfil..." className="mb-6" />
      ) : feedback ? (
      ...
      <div className="grid gap-6 xl:grid-cols-[0.84fr_1.16fr]">
        ...
        <div className="space-y-6">
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button type="submit" disabled={submitting} className="primary-btn inline-flex items-center gap-2">
          ...
          <SectionCard
            ...
            description="Actualiza tu acceso con tu contraseña actual."
            Icon={FiLock}
          >
            {passwordFeedback ? (
              <FeedbackBanner type={passwordFeedback.type} message={passwordFeedback.message} className="mb-4" />
            ) : null}

            <form onSubmit={handlePasswordSubmit} className="space-y-4">
              <div className="grid gap-4 md:grid-cols-3">
                <div>
                  <label className="text-sm font-medium text-slate-600">Contraseña actual</label>
                  <input
                    type="password"
                    name="passwordActual"
                    value={passwordForm.passwordActual}
                    onChange={handlePasswordChange}
                    className={`field-control ${passwordErrors.passwordActual ? 'border-rose-300 focus:border-rose-400 focus:shadow-[0_0_0_4px_rgba(244,63,94,0.12)]' : ''}`}
                  />
                  {passwordErrors.passwordActual ? <p className="mt-2 text-sm font-medium text-rose-600">{passwordErrors.passwordActual}</p> : null}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handlePasswordChange}
                    className={`field-control ${passwordErrors.nuevaPassword ? 'border-rose-300 focus:border-rose-400 focus:shadow-[0_0_0_4px_rgba(244,63,94,0.12)]' : ''}`}
                  />
                  {passwordErrors.nuevaPassword ? <p className="mt-2 text-sm font-medium text-rose-600">{passwordErrors.nuevaPassword}</p> : null}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handlePasswordChange}
                    className={`field-control ${passwordErrors.confirmarPassword ? 'border-rose-300 focus:border-rose-400 focus:shadow-[0_0_0_4px_rgba(244,63,94,0.12)]' : ''}`}
                  />
                  {passwordErrors.confirmarPassword ? <p className="mt-2 text-sm font-medium text-rose-600">{passwordErrors.confirmarPassword}</p> : null}
                </div>
              </div>

              <div className="flex flex-wrap items-center gap-3">
                <button type="submit" disabled={changingPassword} className="primary-btn inline-flex items-center gap-2">
                  <FiLock className="text-base" />
                  {changingPassword ? 'Actualizando...' : 'Actualizar contraseña'}
                </button>
                {passwordSaved ? <span className="soft-chip soft-chip--cool">Contraseña actualizada</span> : null}
              </div>
            </form>
          </SectionCard>
        </div>
